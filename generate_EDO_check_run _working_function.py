...
def search_codes_across_documents(
    ...
    if not remaining or not traceability_documents:
        ...
        # Accumulates EVERY document's real-content match per code, so a code
    # that legitimately (or spuriously) matches in more than one
    # traceability document is resolved by disambiguation, not by
    # whichever document happened to be searched first.
    real_matches: Dict[str, List[dict]] = {}
    ...
    def _consider_match(code, candidate_rows, doc_index, doc_name):
        ...
        if _has_real_traceability_content(row):
            real_matches.setdefault(code, []).append(
                {"row": row, "document_index": doc_index, "document_name": doc_name}
            )
            if len(candidate_rows) > 1:
                logging.info(
                    f"  CANDIDATE | Code: {code} - {len(candidate_rows)} "
                    f"row(s) in document #{doc_index} ({doc_name})."
                )
            # NOTE: do NOT return True here, and do NOT warm prefix_cache
            # yet - a match found here is not final until every remaining
            # document has also been tried for this code.
            return False
    ...
    for doc_index, cached_codes in by_cached_index.items():
        ...
        rows_by_code = _query_traceability_details_chunked(
            pipeline_config, collection, prompt_data, cached_codes,project_code
        )
        logging.info(
    ...
    for doc_index, doc in enumerate(traceability_documents):
        ...
        rows_by_code = _query_traceability_details_chunked(
            pipeline_config, collection, prompt_data, remaining,project_code
        )
        logging.info(
        ...
        # ---- Resolve codes that matched real content in ONE OR MORE
    # documents. A code matching exactly one document is accepted
    # outright. A code matching more than one document is ambiguous -
    # never silently take the first one; disambiguate the same way
    # resolve_best_traceability_row() does within a single document
    # (PROJECT ID match, then "Current" status), and only warm
    # prefix_cache when the winner is unambiguous, so a genuinely
    # ambiguous code never poisons later EDOs' cache lookups.
    for code, matches in real_matches.items():
        if len(matches) == 1:
            winner = matches[0]
            prefix = _code_prefix_hint(code)
            if prefix:
                prefix_cache[prefix] = winner["document_index"]
        else:
            logging.warning(
                f"CALL 7: code {code!r} matched real content in "
                f"{len(matches)} different traceability documents: "
                f"{[(m['document_index'], m['document_name']) for m in matches]} "
                "- disambiguating instead of taking the first."
            )
            project_code_norm = _normalize_project_code(project_code)
            cp_matches = [
                m for m in matches
                if project_code_norm and _normalize_project_code(
                    get_llm_value(
                        m["row"], "Project_ID", "Project Id", "PROJECT ID",
                        "Project_Code", "Project Code", "CP", "cp"
                    )
                ) == project_code_norm
            ]
            pool = cp_matches or matches

            current_matches = [
                m for m in pool
                if "current" in normalize_text(
                    get_llm_value(
                        m["row"], "Record_Status", "Record Status",
                        "RECORD STATUS", "Status", "status"
                    )
                ).lower()
            ]
            pool = current_matches or pool

            winner = pool[0]
            logging.warning(
                f"CALL 7: code {code!r} resolved to document "
                f"#{winner['document_index']} ({winner['document_name']}) "
                "after disambiguation - NOT caching this prefix, since it "
                "was ambiguous."
            )
            # Deliberately skip prefix_cache warming here - an ambiguous
            # code's resolution shouldn't steer every later code sharing
            # its prefix toward a document that wasn't uniquely correct.

        resolved[code] = winner
