...
def extract_existing_edo_verification_details(
    ...
    if "edo_fmea" in edo_document:
        ...
        # FMEA is not present in the resolved edo_document dictionary for
        # this product/template - fall back to RA&C as the document
        # identity for verification reference extraction instead of
        # failing outright.
        logging.warning(
...
def extract_new_edo_verification_details(
    client,
    product_family,
    product,
    templatename,
    ...
    """
    Mirrors extract_existing_edo_verification_details() EXACTLY, but for
    New EDO records: ONE bulk LLM call with every RA/FMEA pair in
    edo_new_data as targets (not one call per record), same prompt
    ("EDO_NEW_Verification_details"), same document-identity fallback
    (edo_fmea, else edo_ra_c).
    """
    logging.info("=" * 80)
    logging.info("STAGE 4c: NEW EDO - VERIFICATION REFERENCE EXTRACTION (BULK, MIRRORS EXISTING EDO)")
    logging.info("=" * 80)

    if not edo_new_data:
        logging.info("No new EDO records to extract verification details for.")
        return {}
    ...
    if "edo_fmea" in edo_document:
        ...
        logging.warning(
            "EDO_FMEA document/collection not found in edo_document - "
            "falling back to EDO_RA_C (RA&C) as the document identity "
            "for New EDO verification reference extraction."
        )
        verification_source_key = "edo_ra_c"
    else:
        raise Exception(
            "Neither EDO_FMEA nor EDO_RA_C document/collection is "
            "configured - cannot run New EDO verification reference extraction."
        )
    ...
    targets = "\n".join(
        f"RA_Number : {entry.get('RA_Number')}\nFMEA_Number : {entry.get('FMEA_Number')}"
        for entry in edo_new_data.values()
        if entry.get("RA_Number") not in (None, "", "Blank") or entry.get("FMEA_Number") not in (None, "", "Blank")
    )

    if not targets:
        logging.warning(
            "No RA/FMEA numbers available on any New EDO record - "
            "skipping verification reference extraction."
        )
        return {}
...
def apply_new_edo_verification(edo_new_data, verification_details):
    """
    Mirrors apply_existing_edo_verification() EXACTLY, but writes onto
    edo_new_data using its own key names: "RA_Number"/"FMEA_Number"
    (capitalized) and "Verification_Reference" - the exact keys
    merge_new_edo_records() already reads via get_llm_value().
    """
    records = deep_extract_records(verification_details)
    ...
    for row in records:
        ...
        for entry in edo_new_data.values():
            entry_fmea = normalize_id(entry.get("FMEA_Number") or "")
            entry_ra = normalize_id(entry.get("RA_Number") or "")

            fmea_match = trace_fmea and entry_fmea and (trace_fmea == entry_fmea or trace_fmea in entry_fmea or entry_fmea in trace_fmea)
            ra_match = trace_ra and entry_ra and (trace_ra == entry_ra or trace_ra in entry_ra or entry_ra in trace_ra)

            if fmea_match or ra_match:
                entry["Verification_Reference"] = verification
...
def generate_edo_template(
    ...
    try:
        ...
        # ---------------------------------------------------
        # STAGE 4C: New EDO Verification Reference (bulk, mirrors
        # Existing EDO's Stage 3B exactly)
        # ---------------------------------------------------
        try:
            new_edo_verification_details = extract_new_edo_verification_details(
                client,
                product_family,
                product,
                templatename,
            ...
            edo_new_data = apply_new_edo_verification(
                edo_new_data,
                new_edo_verification_details
            )

        except Exception as new_verification_error:
            logging.warning(
                "NEW EDO VERIFICATION REFERENCE EXTRACTION SKIPPED - "
                f"Reason: {new_verification_error}"
            )
        print(f"extract_new_edo_verification_details: ", edo_new_data)

        # new_records MUST be defined here - Stage 5's merge_new_edo_records()
        # below depends on it.
        new_records = list(edo_new_data.values())

        # ---------------------------------------------------
        # STAGE 4D: New EDO Diagram Extraction (content-blind, FIFO
        # into Column H - no RA/FMEA matching inside the PDF at all)
        # ---------------------------------------------------
        try:
